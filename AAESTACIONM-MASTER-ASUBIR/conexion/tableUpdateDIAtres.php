...
// Función para promediar máximo y mínimo de la temperatura en un solo día
$varprimerafecha = null;
$arrayfechasphp = [];

$pdo = Database::connect(); // Abrir la conexión una vez
...
for ($i = 0; $i < count($data); $i++) {
    if ($data[$i]['date'] !== $varprimerafecha) {
        ...
        $qCheck->execute(array($fechaaa));
        $count = $qCheck->fetchColumn();

        if ($count == 0) {
            // Insertar solo si el registro no existe
            $sqlInsert = "INSERT INTO esp32_01_tableupdatedia (tempmax, tempmin, hummax, hummin, veleta, anemometro, pluviometro, fecha) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
            $qInsert = $pdo->prepare($sqlInsert);
            $qInsert->execute(array($max_temp, $min_temp, $max_humidity, $min_humidity, $max_veleta, $max_anemometro, $max_pluviometro, $fechaaa));
        }

        $varprimerafecha = $data[$i]['date'];
    }
}
Database::disconnect(); // Cerrar la conexión una vez
?>
